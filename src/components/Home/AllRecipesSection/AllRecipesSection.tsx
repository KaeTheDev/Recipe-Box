...
export default function AllRecipesSection() {
  ...
  useEffect(() => {
    ...
    if (maxTime === "< 30 min") result = result.filter(r => r.prepTime + r.cookTime <= 30);
    else if (maxTime === "< 60 min") result = result.filter(r => r.prepTime + r.cookTime <= 60);

    if (tab === "Favorites") result = result.filter(r => r.isFavorite);
    else if (tab === "Recent") result = result.slice(0, 8);
    else if (tab === "Time")
      result = [...result].sort(
  ...
  return (
    <section className="bg-orange-50 dark:bg-orange-900 py-10 px-4">
      <div className="mx-auto w-full max-w-5xl flex flex-col gap-6">
        {/* Header */}
        <div>
          <h3 className="text-2xl text-gray-900 dark:text-gray-100">All Recipes</h3>
          {!isLoading && (
            <p className="text-sm text-gray-600 dark:text-gray-300">{filtered.length} recipes</p>
          )}
        </div>
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-200 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400"
          />

          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute z-10 mt-1 w-full rounded-lg border bg-white dark:bg-gray-800 shadow-md">
              {suggestions.map((s, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSuggestionClick(s)}
                  className="w-full px-4 py-2 text-left text-sm hover:bg-orange-50 dark:hover:bg-orange-700"
                >
                  {s.startsWith("#") ? <span className="text-orange-600">{s}</span> : s}
                </button>
              ))}
            </div>
          )}
        ...
        <div className="rounded-lg border bg-white dark:bg-gray-800 p-4 shadow-sm">
          <div className="grid gap-4 sm:grid-cols-3">
            <FilterSelect
              ...
              value={cuisine}
              onChange={setCuisine}
              options={["All","Italian","Mexican","Asian","Thai","Japanese","French","Indian","Mediterranean"]}
            />
            <FilterSelect
              label="Difficulty"
              value={difficulty}
  ...
  return (
    <div>
      ...
      <select
        ...
        className="w-full rounded-md border p-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100"
      >
